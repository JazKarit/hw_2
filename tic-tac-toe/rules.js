...
function update_turn_text(){
	document.getElementById("turn_info").innerHTML = "Turn for: <b>"+get_move_symbol()+"</b>";
}
...
function begin_play(){
	var player1_form = document.getElementById("player1_id");
	var player2_form = document.getElementById("player2_id");
	if(isEmpty(player1_form.value) || isEmpty(player2_form.value)){
		alert("Both player names must be filled before starting game");
	}else if (!game_started()){
		player1_form.disabled = true;
		player2_form.disabled = true;
		player1_form.value += " (X)"
		player2_form.value += " (O)"
		update_turn_text()
		started = true;
	}else{
...
function playernum_to_symbol(playernum){
	if(playernum == 1){
		return "X";
	}else{
		return "O";
...
function check_win_game(){
	var counter = 0;
	var last_cell_claim = -1;

	//Follow different paths and increment counter if the claim is the same as
	//the previous claim. If counter reaches 3, we have 3 in a row.

	//Check columns
	for(i = 0; i < 3; i++){
		for(j = 0; j < 3; j++){
			var cell_claim = board_state[i + 3*j]
			console.log(i,j,cell_claim)
			if(cell_claim != last_cell_claim){
				counter = 0
			}
			if(cell_claim == 1){
				last_cell_claim = 1;
				counter++;
			}else if(cell_claim == 0){
				last_cell_claim = 0;
				counter++;
			}else{
				last_cell_claim = -1;
			...
			if(counter == 3){
				alert("Winner is: " + playernum_to_symbol(cell_claim));
				reset_play();
			}
		}
		cell_claim = -1;
	...
	//check diagonals
	for(j = 0; j < 3; j++){
		var cell_claim = board_state[j + 3*j]
		//reset counter if the chain is broken
		if(cell_claim != last_cell_claim){
			counter = 0
		}
		if(cell_claim == 1){
	...
	for(j = 0; j < 3; j++){
		var cell_claim = board_state[j + 3*(2-j)]
		//reset counter if the chain is broken
		if(cell_claim != last_cell_claim){
			counter = 0
		}
...
function isAvailable(cell){
	return board_state[table_ids.indexOf(cell)] == -1;
}
/*
TODO - Rule 3
...
function play() {
	if(!game_started()){
		alert("Please start game before entering moves!")
	}else{
		var move = document.getElementById("move_text_id").value;
		if(!table_ids.includes(move)){
			alert("Please enter a valid cell");
		}else if(!isAvailable(move)){
			alert("Please enter a cell that is not taken");
		}else{
			document.getElementById(move).innerHTML = get_move_symbol();
